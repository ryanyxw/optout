...
def setup_model(args, components):
    model = AutoModelForCausalLM.from_pretrained("./codeparrot-ds-accelerate")
    return model

def setup_optimizer(args, components):
    pass
def setup_tokenizer(args):
    tokenizer = AutoTokenizer.from_pretrained("./codeparrot-ds-accelerate")
    return tokenizer
...
def main(args):
    ...
    prompt_str = "if (lorena == 'wife')"

    tokenized_str = components["tokenizer"](prompt_str, return_tensors="pt")
    print(type(tokenized_str))
    components["model"].eval()
    output = components["model"].generate(**tokenized_str)
    print(components["tokenizer"].decode(output[0]))
